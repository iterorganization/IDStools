...
def map__GEQDSK_to_ids(geqdsk, eq):
    ...
    eq.time_slice[0].global_quantities.magnetic_axis.b_field_phi = (gdsk["FPOL"][0] * coef["sigma_B0_eff"] / gdsk["RMAXIS"])
    ...
    if psi_1d[0] < psi_1d[-1]:
        f_of_psi = lambda p: np.interp(p, psi_1d, f_1d, right=f_1d[-1])
    else:
        f_of_psi = lambda p: np.interp(p, psi_1d[::-1], f_1d[::-1], right=f_1d[-1])
    for i in range(nw):
